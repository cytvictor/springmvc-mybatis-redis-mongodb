...
public class DateTime {
	...
	public static final int DATE = 3; // 获取日;
	...
	/**
	 * 获取当前的时间，加上随机的正数值(共计36位)
	 * yyyyMMddHHmmsssss
	 * 例如：201405311540000232377964671140576790
	 * @return
	 */
	public static String getTimeToFileName(){
		return sdf.format(new Date())+ ""+ Math.abs((new Random()).nextLong());
	}
	 ...
	 */
	/*
	 * public static Date toDate(Object value) { return parseDate(value,
	 * "yyyy-MM-dd"); }
	 */
	 ...
	 * @return
	 */
	public static String getHoursBetweenTwoTimes(Date dateFrom, Date dateEnd) {
		...
		Long dateCnt = inter / dateMillSec;
		long remainder = inter % dateMillSec;
		if(dateCnt.toString().equals("0")){
			Long minCnt = inter / minSec;
			Long minRem = inter % minSec;
			if(minRem != 0){
				minCnt ++;
			}
			return minCnt+"分钟";
		}else{
			Long minCnt = remainder / minSec;
			if(minCnt.toString().equals("0")){
				return dateCnt+"小时";
			}else{
				return dateCnt+"小时"+minCnt+"分钟";
			}
		}
	}
	
	public static boolean compareTwoTimes(Date dateFrom, Date dateEnd) {
		long begin = dateFrom.getTime();
		long end = dateEnd.getTime();
		long inter = end - begin;
		if(inter>=0){
