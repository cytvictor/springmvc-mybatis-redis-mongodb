...
<head>
	...
	<style>
		.close-uploadImg{display:inline-block;width:10px;height:10px;margin-left:5px;background: url(${ctx }/public/img/common/red-close.png) no-repeat 0 0;}
	</style>
</head>
<body>
		...
		<div class="content">
			...
			<div class="otContent">
				<div class="selectBoxTop">
					...
					<p><b class="red">∗</b>资源类型：
						<label><input type="radio" name="funType" value="FUN_TYPE_VIDEO" onclick="onChangeType(0)" checked="checked">视频</label>
						<label><input type="radio" name="funType" value="FUN_TYPE_PICTURE" onclick="onChangeType(1)">图片</label>
						<label><input type="radio" name="funType" value="FUN_TYPE_TEXT" onclick="onChangeType(2)">文字</label>
					</p>
				</div>
				<div class="selectBox mt20">
					...
					<div id="tagThumb">
						...
						<ul>
							...
							<li id="thumbPrintScreen" style="display:none;">
								...
								<input id="printScreenButtonPrev" type="button" value="前一组图片" class="btn preBtn uploadSm" style="position: static" onclick="prevGroup()"/>
					...
					<div id="tagPicture" style="display: none;">
						<h5><b class="red">∗</b>图片：</h5>
						<ul>
							<li id="imageFile">
								<a class="btn bgBlue uploadFileBtn" href="javascript:;">选择图片</a>
								<div class="uploadFileName"></div>
								<input type="hidden" class="successFileName" value=""/>
								<div class="process clearfix">
									<span class="progress-box">
								...
								<input type="hidden" id="resourceImages" name="resourceImages" value="" />
								<div class="progress_info_image" style="display:none;width:600px;text-align:left;">
									<ul id="progress_info_image">
									</ul>
								</div>
							</li>
						</ul>
					</div>
					<div id="tagText" style="display: none;"><%-- 文字 --%>
						<h5><b class="red">∗</b>内容：</h5>
						<ul>
							<li>
								<textarea id="funContent" name="funContent" style="width:600px;height:300px;" maxlength="1200" ></textarea><span class="gray">可输入1200个汉字</span>
							</li>
						</ul>
					</div>
					<h5>标签：</h5>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			//删除上传图片
			$("#progress_info_image").on('click', '.close-uploadImg', function(){
				var id = $(this).parent(WebUploader).attr("id");
				var valueArr = $("#resourceImages").val().split(";");
				var valuestr = "";
				...
				$(this).parent().remove();
				$("#resourceImages").val(valuestr);
			});
			$('input[name="funType"]')[0].checked = true;
			onChangeType(0);
		});
		...
		function ajaxUpload(sequence) {
			...
			} else if (sequence == 'image') {
				var u = '${ctx}/public/uploadImage.do';
				var maxUploadSizePicture = $("#maxUploadSizePicture").val();
				uploadImage(sequence + "File", u, maxUploadSizePicture);
				$("#imageFile").data("WebUploader").on('beforeFileQueued',function(file){
					var len = $("#progress_info_image").find("li").length;
					if(len >= 50){
						Win.alert({
							id : "dialogAlert",
							html : '<span class="dialog_Inner">图片上传超过上限50张</span>',
							width: 300
						});
						return false;
					}
		...
		<%-- 资源类型：  视频  图片  文字 --%>
		var funType = 0;
		var thumbDiv_1 = $("#tagThumb").html();
		var thumbDiv_2 = $("#tagThumb").html();
		function onChangeType(type){
			funType = type;
			if (type == 0){
				//$("input[name=funType]")[0].click() ;
				$("input[value=FUN_TYPE_VIDEO]").attr("checked", "checked") ;
				thumbDiv_2 = $("#tagThumb").html();
				$("#tagThumb").html(thumbDiv_1);
				$("#tagThumb").show();
				$("#tagTip").show();
				$("#tagVideo").show();
				$("#tagPicture").hide();
			...
			if (type == 1){
				$("input[value=FUN_TYPE_PICTURE]").attr("checked", "checked") ;
				thumbDiv_1 = $("#tagThumb").html();
				$("#tagThumb").html(thumbDiv_2);
				cancelSelect();
				$("#tagThumb").show();
				$("#tagTip").hide();
				$("#tagVideo").hide();
			...
			if (type == 2){
				$("input[value=FUN_TYPE_TEXT]").attr("checked", "checked") ;
				$("#tagThumb").hide();
				$("#tagVideo").hide();
				$("#tagPicture").hide();
				$("#tagText").show();
		...
		function onAddResource(){
			...
			if(getValue($("#resourceCatalogRoot").combobox("getValue")) == null || getValue($("#resourceCatalogRoot").combobox("getValue")) == ''){
				Win.alert("请选择关联资源分类!");
				return;
			}else{
				if($(".resourceCatalogSecond").length>0){
					 if($(".resourceCatalogSecond").combobox("getValue") == null|| $(".resourceCatalogSecond").combobox("getValue") == ''){
						 Win.alert("请选择关联资源分类!");
						return;		 
					 }
				 }
			...
			if (funType == 0 && (typeof(highdefineName) == "undefined" || highdefineName == '') && (typeof(middledefineName) == "undefined" || middledefineName == '')){
				Win.alert("请先上传视频文件!");
				return;
			}
			...
			if ((funType == 0) && thumbName == '' && thumbSequence == '') {
				Win.alert("请先上传缩略图或选择一张视频截图!");
				return;
			}
			
			if ((funType == 1) && thumbName == '') {
				Win.alert("请先上传缩略图!");
				return;
			}
			
			if ((funType == 1)) {
				var resourceImages = $("#resourceImages").val();
				if (resourceImages == '') {
					Win.alert("请先至少上传一张图片!");
					return;					
				}
				var imagesLength = resourceImages.split(";");
				if(imagesLength.length>50){
					Win.alert("图片最多只能上传50张!");
					return;	
				}
			}
			
			if (funType == 2){
				if ($.trim($("#funContent").val()) == '') {
					Win.alert("请输入文字内容!");
					return;
				} else if ($("#funContent").val().length > 1200) {
					Win.alert("文字内容只能输入1200个字");
					return;
				}
			}
