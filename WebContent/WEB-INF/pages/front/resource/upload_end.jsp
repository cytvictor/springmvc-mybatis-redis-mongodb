	...
	<div class="container">
		...
		<div class="submitBtn">
			<c:if test="${empty error}">
				<h2 style="font-size:16px;margin: 0 10px 20px 10px">上传资源成功 </h2>
				<input type="button" value="继续上传" class="btn mr10" onclick="window.location='${ctx}/front/resUpload/${module }/upload.html'"/>
			</c:if>
			<c:if test="${not empty error}">
				<h2 style="font-size:16px;margin: 0 10px 20px 10px">上传错误:${error} </h2>
				<input type="button" value="重新上传" class="btn mr10" onclick="window.location='${ctx}/front/resUpload/${module }/upload.html'"/>
			</c:if>
