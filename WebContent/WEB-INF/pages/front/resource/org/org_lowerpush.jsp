					...
					<div class="sourceList2 clearfix">
						...
						<div class="sourceDetail xelIcon">
							<div class="sourceEdit">
								<c:if test="${push.status eq 'REVIEW_NOT_REVIEW' or push.status eq 'REVIEW_ACCEPT'}">
									...
									<c:if test="${push.status eq 'REVIEW_ACCEPT' and empty push.nextResourcePushId and sessionScope.session_login_user.orgLevel ne '1'}">
										<a href="javascript:pushedPushOrg('${push.id}');" class="">推荐给上级</a>
									</c:if>
									<c:if test="${push.status eq 'REVIEW_ACCEPT' and empty push.distributeId}">
										<a href="javascript:pushedDistributeTeacher('${push.id}');">分发给下级</a>
									</c:if>
								</c:if>
							</div>
							<ul>
						...
						<div class="sourceState">
							...
							<c:if test="${not empty push.distributeId}">
								<span class="red waitCheck">
								<c:if test="${push.resourceDistribute.status eq 'REVIEW_NOT_REVIEW'}">分发给下级待审核</c:if>
								<c:if test="${push.resourceDistribute.status eq 'REVIEW_ACCEPT'}">已分发给下级</c:if>
								<c:if test="${push.resourceDistribute.status eq 'REVIEW_REJECT'}">分发给下级审核不通过</c:if>
								</span>
							</c:if>
						</div>
					</div>
					...
					<script type="text/javascript">
						...
						function pushedDistributeTeacher(id){
							Win.open({
								...
								width:600,
								height:400,
								url:"${ctx}/front/distribute/orgAndSchoolSelect.html?id=" + id + "&action=pushedToSchoolAndOrg",
